<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>

<!DOCTYPE html>
<html lang="en">

<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Document</title>
<link rel="stylesheet"
	href="${pageContextpath.request.contextPath }/assets/css/main-search.css">
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>

</head>
<%@ include file="header.jsp"%>
<body>
	<div>
		<form>
			<div class="results_board">
				<p class="top_title">커뮤콕콕</p>
				<div class="conts">
					<div class="content">
						<span class="title"> <a href="#">내일 모바일게임 대작 2개 출시하네요.
								<font class="comment-cnt">15</font>
						</a>
						</span>
						<p>
							<a href="#"><b>호연</b> 영웅전설은 30대 이상만 아는 추억의 게임 &nbsp;</a>
						</p>
						<p class="desc">
							<span>[자유게시판]</span> | <span>2024.08.27</span> | <span
								class="like">0</span> |
						</p>
					</div>
					<div class="content">
						<span class="title"> <a href="#">내일 모바일게임 대작 2개 출시하네요.
								<font class="comment-cnt">15</font>
						</a>
						</span>
						<p>
							<a href="#"><b>호연</b> 영웅전설은 30대 이상만 아는 추억의 게임 &nbsp;</a>
						</p>
						<p class="desc">
							<span>[자유게시판]</span> | <span>2024.08.27</span> | <span
								class="like">0</span> |
						</p>
					</div>
					<div class="content">
						<span class="title"> <a href="#">내일 모바일게임 대작 2개 출시하네요.
								<font class="comment-cnt">15</font>
						</a>
						</span>
						<p>
							<a href="#"><b>호연</b> 영웅전설은 30대 이상만 아는 추억의 게임 &nbsp;</a>
						</p>
						<p class="desc">
							<span>[자유게시판]</span> | <span>2024.08.27</span> | <span
								class="like">0</span> |
						</p>
					</div>
					<!-- 더보기 버튼 -->
					<a href="#" class="more-button">커뮤 더보기!</a>
					<!-- 추가적인 검색 결과 항목을 여기에 추가하세요 -->
					<p class="top_title">핫플콕콕</p>
					<div class="conts">
						<div class="content">
							<span class="title"> <a href="#">내일 모바일게임 대작 2개 출시하네요.
									<font class="comment-cnt">15</font>
							</a>
							</span>
							<p>
								<a href="#"><b>호연</b> 영웅전설은 30대 이상만 아는 추억의 게임 &nbsp;</a>
							</p>
							<p class="desc">
								<span>[자유게시판]</span> | <span>2024.08.27</span> | <span
									class="like">0</span> |
							</p>
						</div>
						<div class="conts">
							<div class="content">
								<span class="title"> <a href="#">내일 모바일게임 대작 2개
										출시하네요. <font class="comment-cnt">15</font>
								</a>
								</span>
								<p>
									<a href="#"><b>호연</b> 영웅전설은 30대 이상만 아는 추억의 게임 &nbsp;</a>
								</p>
								<p class="desc">
									<span>[자유게시판]</span> | <span>2024.08.27</span> | <span
										class="like">0</span> |
								</p>
							</div>
							<div class="conts">
								<div class="content">
									<span class="title"> <a href="#">내일 모바일게임 대작 2개
											출시하네요. <font class="comment-cnt">15</font>
									</a>
									</span>
									<p>
										<a href="#"><b>호연</b> 영웅전설은 30대 이상만 아는 추억의 게임 &nbsp;</a>
									</p>
									<p class="desc">
										<span>[자유게시판]</span> | <span>2024.08.27</span> | <span
											class="like">0</span> |
									</p>
								</div>
							</div>
						</div>
						<!-- 더보기 버튼 -->
						<a href="#" class="more-button">핫플 더보기!</a>
						<p class="top_title">편의공공</p>
						<div class="conts">
							<div class="content">
								<span class="title"> <a href="#">내일 모바일게임 대작 2개
										출시하네요. <font class="comment-cnt">15</font>
								</a>
								</span>
								<p>
									<a href="#"><b>호연</b> 영웅전설은 30대 이상만 아는 추억의 게임 &nbsp;</a>
								</p>
								<p class="desc">
									<span>[자유게시판]</span> | <span>2024.08.27</span> | <span
										class="like">0</span> |
								</p>
							</div>
							<div class="conts">
								<div class="content">
									<span class="title"> <a href="#">내일 모바일게임 대작 2개
											출시하네요. <font class="comment-cnt">15</font>
									</a>
									</span>
									<p>
										<a href="#"><b>호연</b> 영웅전설은 30대 이상만 아는 추억의 게임 &nbsp;</a>
									</p>
									<p class="desc">
										<span>[자유게시판]</span> | <span>2024.08.27</span> | <span
											class="like">0</span> |
									</p>
								</div>
								<div class="conts">
									<div class="content">
										<span class="title"> <a href="#">내일 모바일게임 대작 2개
												출시하네요. <font class="comment-cnt">15</font>
										</a>
										</span>
										<p>
											<a href="#"><b>호연</b> 영웅전설은 30대 이상만 아는 추억의 게임 &nbsp;</a>
										</p>
										<p class="desc">
											<span>[자유게시판]</span> | <span>2024.08.27</span> | <span
												class="like">0</span> |
										</p>
									</div>
								</div>
							</div>
							<!-- 더보기 버튼 -->
							<a href="#" class="more-button">편의 더보기!</a>
							<div class="info-bg2">
								<div class="bottom-list">
									<a class="prev" onfocus="blur()" href="#"></a> <a
										class="num on" onfocus="blur()" href="#">1</a> <a class="num"
										onfocus="blur()" href="#">2</a> <a class="num"
										onfocus="blur()" href="#">3</a> <a class="num"
										onfocus="blur()" href="#">4</a> <a class="num"
										onfocus="blur()" href="#">5</a> <a class="next"
										onfocus="blur()" href="#"></a>
								</div>
							</div>
		</form>
	</div>
</body>
<%@ include file="footer.jsp"%>

</html>