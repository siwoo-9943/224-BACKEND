...
document.getElementById('main-picked').addEventListener('click', function() {
	// 현재 스크롤 위치 저장
	const scrollPosition = window.scrollY;

	// AJAX 요청으로 서버에 스크롤 위치 전송
	fetch('/saveScrollPosition', {
		method: 'POST',
		headers: {
		...
		.then(response => {
			if (response.ok) {
				// 성공적으로 저장되면 원하는 작업 수행
				console.log('스크롤 위치 저장 성공');
			}
		})
