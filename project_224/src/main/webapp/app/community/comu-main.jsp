		...
		<div class="wrapper">
			<div class="contents">
				...
				<div class="navcontainer">
					<ul id="navlist">
						<li><a href="?category=전체"
							class="category-filter ${selectedCategory eq '전체' ? 'active' : ''}"
							data-category="전체">전체</a></li>
						<li><a href="?category=자유"
							class="category-filter ${selectedCategory eq '자유' ? 'active' : ''}"
							data-category="자유">자유</a></li>
						<li><a href="?category=핫플 자유"
							class="category-filter ${selectedCategory eq '핫플 자유' ? 'active' : ''}"
							data-category="핫플 자유">핫플 자유</a></li>
						<li><a href="?category=핫플 후기"
							class="category-filter ${selectedCategory eq '핫플 후기' ? 'active' : ''}"
							data-category="핫플 후기">핫플 후기</a></li>
						<li><a href="?category=편공 자유"
							class="category-filter ${selectedCategory eq '편공 자유' ? 'active' : ''}"
							data-category="편공 자유">편공 자유</a></li>
						<li><a href="?category=편공 후기"
							class="category-filter ${selectedCategory eq '편공 후기' ? 'active' : ''}"
							data-category="편공 후기">편공 후기</a></li>
					</ul>
					...
					<c:forEach var="community" items="${communityList}">
						<li class="baseList-item">
							<div class="baseList-space">
								...
								<c:if test="${empty community.memberNickname}">
									<span class="baseList-sector">탈퇴한 회원입니다</span>
								</c:if>
								<span class="baseList-sector">${community.boardRegistDate}</span>
								<span class="baseList-sector">${community.recommendCount}</span>
							</div>
				...
				<div class="pagination-container">
					<ul class="community-pagenation">
						<c:if test="${currentPage > 1}">
							<li><a
								href="?page=${currentPage - 1}&category=${selectedCategory}">&lt;</a></li>
						</c:if>
