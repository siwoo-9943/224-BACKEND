...
public class MypageOkController  implements MemExecute {
	...
	public Result MemExecute(HttpServletRequest request, HttpServletResponse response)
		...
		String myprofile = mypageDAO.myprofile(memberEmail, memberName,memberNickName,memberPhone,memberGender,memberBirth);
		...
		// 입력값이 디비에 존재하는지 확인
					...
					String mypage1 = mypageDAO.myprofile(memberEmail, memberName,memberNickName,memberPhone,memberGender,memberBirth);
					System.out.println(mypage1 + "보낼값들어오는지확인");
					request.setAttribute("mypage1", mypage1);
					
					request.getRequestDispatcher("/app/mypage/my-profile.jsp").forward(request, response); // 아이디 찾기 성공 후 프컨타고 아이디결과페이지로 이동
