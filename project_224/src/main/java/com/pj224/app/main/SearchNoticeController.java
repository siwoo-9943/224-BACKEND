...
public class SearchNoticeController implements MemExecute{
	...
	public Result MemExecute(HttpServletRequest request, HttpServletResponse response)
	    ...
	    List<MainDTO> searchnoResults;

	    // 검색어가 비어있지 않은 경우에만 검색 수행
	    if (searchInput != null && !searchInput.trim().isEmpty()) {
	        // 검색 결과 가져오기
	        searchnoResults = mainDAO.searchnoInfo(searchInput);
	        System.out.println("컨트롤러 검색어 : " + searchInput);
	        System.out.println("검색! \n" + searchnoResults);
	    } else {
	        // 검색어가 비어있을 경우 전체 리스트 가져오기
	        searchnoResults = mainDAO.searchnoList();
	        System.out.println(searchnoResults);
	        System.out.println("검색값 없음!");
	    }

	    // 검색 결과를 request에 저장
	    request.setAttribute("searchnoResults", searchnoResults);
